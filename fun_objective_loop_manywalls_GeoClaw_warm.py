...
def objective(x,SVf1,SVf2,SVf3,SVf4,SVf5,SVf6,SVf7,SVf8,SVf9,SVf10,SVf11,SVf12,SVf13,SVf14,SVf15,SVf16,SVf17,SVf18,SVf19,SVf20,
    ...
    for N in range(numiter):
        ...
        peak_w = pd.read_csv(r"SurgeDataGeo/%d-peak_w.csv"%N).values
        #surge_c = pd.read_csv(r"SurgeData/%d-waterlevel_c.csv"%N).values
        surge_w = pd.read_csv(r"SurgeDataGeo/%d-waterlevel_w.csv"%N).values
        #time_c = pd.read_csv(r"SurgeData/%d-time_c.csv"%N).values
        time_w = pd.read_csv(r"SurgeDataGeo/%d-time_w.csv"%N).values
        ...
        #fld_h_c = np.zeros((np.shape(cpi1_c)[0],ndiv18)); 
        fld_h_w = np.zeros((np.shape(cpi1_w)[0],ndiv18))
        #V_c = np.zeros((np.shape(cpi1_c)[0],ndiv18)); 
        V_w = np.zeros((np.shape(cpi1_w)[0],ndiv18))
        ...
        #n_damage_loss_c = np.append(n_damage_loss_c,np.sum(damage_loss_c));   
        n_damage_loss_w = np.append(n_damage_loss_w,np.sum(damage_loss_w))
        #n_cost_util_c   = np.append(n_cost_util_c,np.sum(cost_util_c));       
        n_cost_util_w   = np.append(n_cost_util_w,np.sum(inop_util_w))
        #n_cost_tran_c   = np.append(n_cost_tran_c,np.sum(cost_tran_c));       
        ...
        n_cost_direct_sum_div.append(np.array(df_cost_direct_sum_div_w))



    #mean_damage_loss_c  = np.mean(n_damage_loss_c); 
    mean_damage_loss_w  = np.mean(n_damage_loss_w)
    #mean_cost_util_c    = np.mean(n_cost_util_c);   
    mean_cost_util_w    = np.mean(n_cost_util_w)
    #mean_cost_tran_c    = np.mean(n_cost_tran_c);   
    mean_cost_tran_w    = np.mean(n_cost_tran_w)

    return wall_cost  + mean_damage_loss_w  + mean_cost_util_w  + mean_cost_tran_w, wall_cost, n_cost_direct_sum_div
